...
namespace HLTStudio.Commons
{
	...
	public static class SCommon
	{
		...
		/// https://github.com/haru-labo-tech/Factory/blob/master/Common/DataConv.c#L460-L491
		/// </summary>
		...
		/// https://github.com/haru-labo-tech/Factory/blob/master/Common/DataConv.c#L503-L563
		/// </summary>
		...
		/// https://github.com/haru-labo-tech/Factory/blob/master/Common/DataConv.c#L582-L604
		/// </summary>
		...
		/// 以下の関数を踏襲した。(慣習的実装)
		/// https://github.com/haru-labo-tech/Factory/blob/master/Common/DataConv.c#L320-L388
		/// </summary>
		...
		private class JCharCodes
		{
			...
			/// <summary>
			/// generated by https://github.com/haru-labo-tech/Factory/blob/master/Labo/GenData/IsJChar.c
			/// </summary>
			private void AddAll()
			{
		...
		public static string ReplaceAll(string text, params string[] replacements)
		{
			...
			for (int index = 0; index < rPCount; index++)
				text = text.Replace(replacements[index * 2], markers[index]);
		...
		{
			// '{' + 240-bit-CRnd-B32 + '}', 50(48 + 2[括弧]) chars, ★全て大文字
			//       ~~~~~~~~~~~~~~~~
			//       (30 / 5) * 8 = 48 桁
			//
			return $"{{{GetCUID_NB()}}}";
		...
		{
			//       240-bit-CRnd-B32, 48 chars, ★全て大文字
			//       ~~~~~~~~~~~~~~~~
			//       (30 / 5) * 8 = 48 桁
			//
		...
		{
			// '{' + epoch-time-millis-x + '-' + 120-bit-CRnd-B32 + '}', 39(12 + 24 + 3[括弧とハイフン]) chars, ★全て大文字
			//       ~~~~~~~~~~~~~~~~~~~         ~~~~~~~~~~~~~~~~
			//       |                           (15 / 5) * 8 = 24 桁
		...
		{
			//       epoch-time-millis-x + '-' + 120-bit-CRnd-B32, 37(12 + 24 + 1[ハイフン]) chars, ★全て大文字
			//       ~~~~~~~~~~~~~~~~~~~         ~~~~~~~~~~~~~~~~
			//       |                           (15 / 5) * 8 = 24 桁
			//       E677D21FDBFF = 9999/12/31 23:59:59.999 UTC (DateTimeOffset.MaxValue)
			//       ~~~~~~~~~~~~
			//       12 桁
			//
			return $"{SCommon.GetEpochTimeMillisForID():X12}-{SCommon.Base32.I.Encode(SCommon.CRandom.GetBytes(15))}";
		}
		...
		public static void DistinctComp<T>(List<T> list, Comparison<T> comp) // list: 要素を削除するので IList ではなく List
		{
			Distinct(list, (a, b) => comp(a, b) == 0);
		}

		public static void Distinct<T>(List<T> list, Func<T, T, bool> match) // list: 要素を削除するので IList ではなく List
		{
			for (int l = 0; l < list.Count; l++)
				for (int r = l + 1; r < list.Count; r++)
		...
		public static void DistinctSort<T>(List<T> list, Comparison<T> comp) // list: 要素を削除するので IList ではなく List
		{
			if (list.Count < 2)
				return;
		...
		private class P_AS_IndexedValue<T>
		{
			public int Index;
			public T Value;
		}
		...
		public class SortedArray<T>
		{
			...
			/// <summary>
			/// SCommon.GetIndex, SCommon.GetRange の仕様により第2引数が key になることを保障する。
			/// </summary>
			...
			/// N/A
			/// </summary>
			/// <param name="takenElements">N/A</param>
			/// <param name="comp">SCommon.GetIndex, SCommon.GetRange の仕様により第2引数が key になることを保障する。</param>
			public SortedArray(IList<T> takenElements, Comparison<T> comp)
			{
				SCommon.Sort(takenElements, comp);

				this.Elements = takenElements;
				this.Comp = comp;
			}

			public int GetIndex(T key)
			{
				return SCommon.GetIndex(this.Elements, key, this.Comp);
			}

			public int[] GetRange(T key)
			{
				return SCommon.GetRange(this.Elements, key, this.Comp);
			}
		...
		public class KVArray<K, V>
		{
			...
			/// N/A
			/// </summary>
			/// <param name="keys">N/A</param>
			/// <param name="values">N/A</param>
			/// <param name="comp">SCommon.GetIndex, SCommon.GetRange の仕様により第2引数が key になることを保障する。</param>
			public KVArray(K[] keys, V[] values, Comparison<K> comp)
			{
				int count = keys.Length;

				Element_t[] takenElements = new Element_t[count];
				...
				for (int index = 0; index < count; index++)
				{
					takenElements[index] = new Element_t()
					{
						Key = keys[index],
						Value = values[index],
					};
				}
				this.Elements = new SCommon.SortedArray<Element_t>(takenElements, (a, b) => comp(a.Key, b.Key));
			}
		...
		public static T[][] ToSizedRect<T>(T[][] rows, T defval, int rowCount, int columnCount)
		{
			return ToRect(rows, defval, rowCount, columnCount, rowCount, columnCount);
		}

		public static T[][] ToRect<T>(T[][] rows, T defval)
		{
			return ToRect(rows, defval, 0, 0);
		}

		public static T[][] ToRect<T>(T[][] rows, T defval, int rowCountMin, int columnCountMin)
		{
			return ToRect(rows, defval, rowCountMin, columnCountMin, SCommon.IMAX, SCommon.IMAX);
		}

		public static T[][] ToRect<T>(T[][] rows, T defval, int rowCountMin, int columnCountMin, int rowCountMax, int columnCountMax)
		{
			if (
				rows == null ||
			...
			if (
				...
				SCommon.IMAX < rowCountMax ||
				columnCountMin < 0 ||
				columnCountMin > columnCountMax ||
				SCommon.IMAX < columnCountMax
				)
				throw new Exception("Bad range");

			int h = rows.Length;
			int w = h == 0 ? 0 : rows.Max(row => row.Length);

			h = SCommon.ToRange(h, rowCountMin, rowCountMax);
			w = SCommon.ToRange(w, columnCountMin, columnCountMax);
		...
		public static T[][] RowsToBRSlim<T>(T[][] rows_SRC, Predicate<T> matchToDelete)
		{
...
#if false // zantei -- RESLinesTo*系メソッドの使い方メモ
...
Press ENTER key.
		...
		public static RESTree_t RESLinesToTree(string[] lines)
		{
			...
			foreach (string line in lines)
			{
				...
				if (indent + 1 < i)
					throw new Exception("リソースツリーの書式に問題があります。(途中のインデント)");
